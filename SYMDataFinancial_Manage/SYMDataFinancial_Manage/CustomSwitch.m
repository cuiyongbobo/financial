...
-(void)setStatus:(CustomSwitchStatus)status
{
    ...
    _status = status;
        if ([_delegate respondsToSelector:@selector(customSwitchSetStatus:switch:)]) {
            [_delegate customSwitchSetStatus:_status switch:self];
        }
}
...
{
    [UIView animateWithDuration:1/60.0f animations:^{
