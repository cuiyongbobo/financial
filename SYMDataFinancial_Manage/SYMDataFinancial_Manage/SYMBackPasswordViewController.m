...
@interface SYMBackPasswordViewController ()
{
...
@implementation SYMBackPasswordViewController
...
-(void)tapBack:(UITapGestureRecognizer *) tap{
    [UIView animateWithDuration:0.3f animations:^{
        ...
        [self.NewInformationTextField resignFirstResponder];
            ...
            if ([[SYMPublicDictionary shareDictionary]judgeString:self.IPhoneNumber.text]) {
                ...
                if (isb) {
                    if ([[SYMPublicDictionary shareDictionary]judgeString:self.Password.text]) {
                        ...
                        BOOL isc=[self isPassword:self.NewInformationTextField.text];
                        if (isc) {
                            [self setPayPass];
                        }else{
                            dispatch_async(dispatch_get_main_queue(), ^{
...
#pragma mark- 设置登录密码接口
-(void)setPayPass
{
    NSMutableDictionary *paramDict=[[NSMutableDictionary alloc]initWithCapacity:10];
    paramDict=[[SYMPublicDictionary shareDictionary]SetPayPasswordPublicDictnary:[[NSUserDefaults standardUserDefaults]objectForKey:ISLogIN] pwdType:0 mobile:self.IPhoneNumber.text loginPwd:self.NewInformationTextField.text cashPwd:@""];
    NSLog(@"paramDict--->%@",paramDict);
    [SYMAFNHttp post:SYMSetPayPassWord_URL params:paramDict success:^(id responseObj){
        NSDictionary *responsedict = [NSJSONSerialization JSONObjectWithData:responseObj options:NSJSONReadingMutableContainers error:nil];
        NSLog(@"loginresponseObj-->%@",responsedict);
        if ([[NSString stringWithFormat:@"%@",responsedict[@"code"]] isEqualToString:@"1000"]) {
            [[MyTipsWindow shareMytipview]HiddenTipView:NO viewcontroller:self tiptext:@"登录密码找回成功" backgroundcolor:white];
            [self.navigationController popViewControllerAnimated:YES];
        }else{
            [[MyTipsWindow shareMytipview]HiddenTipView:NO viewcontroller:self tiptext:[NSString stringWithFormat:@"%@",responsedict[@"message"]] backgroundcolor:white];
        }
