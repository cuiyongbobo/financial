...
-(NSString *)signUrlStrWith:(NSDictionary *)dic
{
    ...
    // NSString *qStr=[outputAfter substringFromIndex:0];
    
    NSLog(@"qstr--->%@",outputAfter);
    NSString *outStr=[[self md5:outputAfter] uppercaseString];
...
-(NSData *)acquiceDataWithDic:(NSDictionary *)dic
{
    NSArray *sortArray=[dic allKeys];
    NSMutableString *outputAfter = [[NSMutableString alloc] init];
    ...
    NSLog(@"outputAfter=%@",outputAfter);
    
    return [outputAfter dataUsingEncoding:NSUTF8StringEncoding allowLossyConversion:YES];
}
